...
class OptionsMenu:
    def __init__(self, window, volume_settings):
    ...
    def write_volume_settings(self):
        with open("Artifacts_MMO_Client/saved_user_options.py", "w") as file:
            file.write(f"music_volume = {self.volume_settings.music_volume}\n")
            file.write(f"sound_volume = {self.volume_settings.sound_volume}\n")
    ...
    def setup(self):
        ...
        self.music_volume_slider = pygame_gui.elements.UIHorizontalSlider(relative_rect=self.center_ui_element(400, 50, 50), start_value=self.volume_settings.music_volume, value_range=[0, 1], manager=self.gui_manager)
        ...
        self.sound_volume_slider = pygame_gui.elements.UIHorizontalSlider(relative_rect=self.center_ui_element(400, 50, -100), start_value=self.volume_settings.sound_volume, value_range=[0, 1], manager=self.gui_manager)
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame_gui.UI_HORIZONTAL_SLIDER_MOVED:
                    if event.ui_element == self.sound_volume_slider:
                        sound_volume = round(self.sound_volume_slider.get_current_value(), 1)
                        self.volume_settings.sound_volume = sound_volume
                    
                    if event.ui_element == self.music_volume_slider:
                        music_volume = round(self.music_volume_slider.get_current_value(), 1)
                        self.volume_settings.music_volume = music_volume

                if event.type == pygame_gui.UI_BUTTON_PRESSED:
                    self.pygame_utils.play_music(self.button_sound, self.volume_settings.sound_volume, 0)
                    if event.ui_element == self.back_button:
                        self.write_volume_settings()
